...
insert into trag_text(indiv_id,deathtext,eventext)
	...
	string_agg(DISTINCT COALESCE(event.place,' ')||COALESCE(event.notes,' ')||COALESCE(event.cause,' ')||
		COALESCE(event.label,' '),' '), -- as deathtext,
	replace(string_agg(DISTINCT COALESCE(event2.place,' ')||COALESCE(event2.notes,' ')||COALESCE(event2.cause,' ')||
